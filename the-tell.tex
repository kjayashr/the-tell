...
The edges are real. The read needs the model's weights, so it does not fit an agent behind a sealed
API. The direction belongs to one model. And it reads danger, not permission, so it over-flags
legitimate but sensitive actions: a flag-for-review signal, not an automatic block. Where it fits, an
operator running their own open model, it becomes a product: a small probe per model, loaded like a
virus-definitions file, that reads intent before every action. Code and probe:
\texttt{github.com/kjayashr/the-tell}.
\end{abstract}
...
\includegraphics[width=0.74\textwidth]{figs/fig_layers_manim.png}
...
\begin{figure}[htbp]\centering
\includegraphics[width=0.72\textwidth]{figs/fig_projection_manim.png}
...
\begin{figure}[htbp]\centering
\includegraphics[width=0.72\textwidth]{figs/fig_collapse.pdf}
\caption{From many numbers to one. Each request is a long list of numbers, a point in the space up top. The probe collapses that whole list to a single score on the line below, where safe and unsafe fall apart.}\label{fig:collapse}
\end{figure}
...
itself (Figure~\ref{fig:collapse}). The benign requests gather on one side, the malicious ones on the
...
The gap in Figure~\ref{fig:collapse} is not an artifact of one lucky layer. Fitting a plain linear read
...
\emph{evade}, \emph{insidious} (Figure~\ref{fig:vocab}). Several of these surfaced as Chinese tokens,
...
\begin{figure}[htbp]\centering
\includegraphics[width=0.7\textwidth]{figs/fig_vocab.pdf}
\caption{Read back through the model's own vocabulary, the intent direction points at the model's words for wrongdoing.}\label{fig:vocab}
\end{figure}


\section{Why not just read the words?}
...
the words (Figure~\ref{fig:vs}, right). On this task the
...
the words are made to look innocent, only the read of the inside still sees the intent.
\begin{figure}[htbp]\centering
\includegraphics[width=0.8\textwidth]{figs/fig_deception_meters.png}
\caption{When the reply is made to look innocent, a reader of the words is blind ($0.454$), while the activation read still separates safe from unsafe ($0.993$).}\label{fig:deception}
\end{figure}
...
probe from the first model is not even the right size to lay against the second. It is why the product is a library, not a single file.
...
\begin{figure}[htbp]\centering
\includegraphics[width=0.56\textwidth]{figs/fig_radar.pdf}
...
\begin{figure}[htbp]\centering
\includegraphics[width=0.82\textwidth]{figs/fig_virusdef.png}
\caption{The whole artifact is a small per-model file, shipped beside the model the way antivirus ships its definitions.}\label{fig:virusdef}
\end{figure}
...
\begin{figure}[htbp]\centering
\includegraphics[width=0.72\textwidth]{figs/fig_latency_timeline.pdf}
\caption{The read runs inside the forward pass and finishes before the first word of the reply, for under one percent more time.}\label{fig:latency}
\end{figure}
...
\section{Where it breaks}\label{sec:limits}
The probe reads \emph{danger}, not \emph{permission}, and that is the limit that matters most. We stress-tested it on twenty requests that are
...
On a small open model, run on a laptop, an agent's intent is
